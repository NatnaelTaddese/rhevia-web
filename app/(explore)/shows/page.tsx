...
import { getTop10TVShows } from "@/lib/explore-data";
import {
...
async function Top10Section() {
  const items = await getTop10TVShows();

  if (!items.length) return null;

  return <Top10Hero items={items} type="series" />;
}

async function StreamingCatalogsGrid() {
  const [catalogs, providerLogos] = await Promise.all([
    getStreamingCatalogs("series", ALL_PROVIDERS),
    getProviderLogos(),
  ]);
